...
TrainingNames <- readRDS("data/TrainLabelsnopre32.Rda")
TestingNames <- readRDS("data/TestLabelsnopre32.Rda")
...
TrainingSet <- readRDS("data/Full/TrainFullnopre32.Rda")
TestingSet <- readRDS("data/Full/TestFullnopre32.Rda")
...
#model ----
 ...
 (model <- keras_model_sequential() %>% 
     ...
     layer_conv_1d(kernel_size = 15, filters = 7, strides = 2, activation = "relu", name = "3conv") %>% 
     layer_dropout(.5) %>% 
     layer_max_pooling_1d(2) %>% 
     
     layer_conv_1d(kernel_size = 10, filters = 10, strides = 2, activation = "relu", name = "4conv") %>% 
...
table(TestingNames, prediction = predict_classes(model, TestingSet)) # creates a simple confusion matrix
table(TestingNames, TestingNames)
