...
channels <- "rear" #can be 32, 22, 13, 5, 4rear, 4front, mid, left, right or rear
fold <- 1 #can be (1:5)
...
TrainingNames <- readRDS(paste("data2/Split B/", channels, "/", fold, "TrainLabels.Rda", sep = "")
                         )
TestingNames <- readRDS(paste("data2/Split B/",  channels, "/", fold, "TestLabels.Rda", sep = "")
                        )
...
TrainingSet <- readRDS(
  paste("data2/Split B/", channels, "/", fold , "Train.Rda", sep = "")
)
TestingSet <- readRDS(
  paste("data2/Split B/", channels, "/", fold , "Test.Rda", sep = "")
)
